...
def send_email_api():
    ...
    if smtp_host and smtp_user and smtp_pass and to_email:
        try:
            ...
            msg = MIMEText(body)
            msg['Subject'] = f'Purchase Order Request — {supplier}'
            msg['From'] = smtp_user
            msg['To'] = to_email
            with smtplib.SMTP_SSL(smtp_host, 465) as server:
                server.login(smtp_user, smtp_pass)
                server.send_message(msg)
            return jsonify({"status": "sent", "mode": "live", "to": to_email})
        except Exception as e:
            return jsonify({"status": "error", "message": str(e)}), 500

    print(f"[DEMO MODE] Email to {supplier} <{to_email or 'no address'}>:\n{body}\n")
    return jsonify({"status": "sent", "mode": "demo", "note": "SMTP not configured — logged to console."})
